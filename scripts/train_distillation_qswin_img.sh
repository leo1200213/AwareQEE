...
python -m torch.distributed.run --nproc_per_node=2 --master_port=29527 --nnodes=1 ../examples/run_distillation_qswin.py \
    --report_to wandb \
    ...
    --output_dir "$project_root/saved_models/$MODEL_TYPE/$DATASET/${HIGHWAY_TYPE}/stage2_${TRAIN_STRATEGY}/swin/quantized" \
    ...
    --threshold 0.8 \
    --fp16 True\
    --per_w_layer_bits "[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6]" \
    --per_a_layer_bits "[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6]"\
