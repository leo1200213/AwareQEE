...
python -m torch.distributed.run --nproc_per_node=2 --master_port=29577 --nnodes=1 ../examples/run_distillation_deit.py \
    --report_to wandb \
    --threshold 0.8 \
    ...
    --output_dir "$project_root/saved_models/$MODEL_TYPE/$DATASET/${HIGHWAY_TYPE}/stage2_${TRAIN_STRATEGY}/deit/" \
