...
from models.swin_highway.modeling_highway_qswin import SwinHighwayForImageClassification_distillation
...
class TrainerwithExits(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            if hasattr(outputs, "loss") and outputs.loss is not None:
                ...
                # Compute loss manually if not provided by the model
            ...
            if hasattr(outputs, "loss") and outputs.loss is not None:
                ...
                # Handle case when labels are not provided
                loss = None  # Or set to zero, or handle appropriately
    ...
    def evaluation_loop(
        ...
        # Compute BOPs
        total_bops, avg_bops_per_sample, bops_per_exit_layer = self.compute_bops_swin(model.config, exit_counts)
...
def main():
    ...
    output_dir_name = f"Swin_qae_data_{data_args.dataset_name}_quant_{quant_bits}_conf_{confidence}_w_bits_{per_w_layer_bits_str}_a_bits_{per_a_layer_bits_str}_"
