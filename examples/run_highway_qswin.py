...
def combined_risk(highway_logits, final_layer_logits, labels, loss_fn=F.cross_entropy, gamma=0.1):
    """
    A combined risk measure that uses performance gap risk but also rewards correctness.
    If the early exit is correct, risk is reduced, aligning risk more closely with accuracy.
    
    Args:
        highway_logits: Logits from the early exit.
        final_layer_logits: Logits from the final layer of the full model.
        labels: Ground truth labels.
        loss_fn: Loss function for computing performance gap (default: cross entropy).
        gamma: Weight that rewards correctness. Higher gamma gives more emphasis to accuracy.

    Returns:
        Combined risk value: lower if early exit is correct.
    """
    # Compute the performance gap risk
    gap_risk = performance_gap_risk(highway_logits, final_layer_logits, labels, loss_fn=loss_fn)

    # Determine correctness at the early exit
    with torch.no_grad():
        preds = torch.argmax(highway_logits, dim=-1)
        # correct_rate is the fraction of samples correctly predicted in the batch
        correct_rate = (preds == labels).float().mean()
    print(f"gap_risk={gap_risk},corr ect_rate={correct_rate}")
    # Reduce risk by gamma * correct_rate to reward accuracy
    combined = - correct_rate
    return combined
...
def approximate_quantized_model_size_staged(model):
    """
    Example: If model.config has per_w_layer_bits = [8, 8, 4, 4]
    for 4 Swin stages, parse out which stage the param belongs to,
    then multiply by the correct bitwidth.
    """
    if not hasattr(model.config, "per_w_layer_bits"):
        raise ValueError("model.config.per_w_layer_bits is not defined.")

    per_stage_bits = model.config.per_w_layer_bits  # e.g. [8, 8, 4, 4]
    total_bits = 0

    for name, param in model.named_parameters():
        # If you want to skip classifier:
        if "classifier" in name:
            continue

        # Parse stage index from name (e.g. "encoder.layers.2.")
        match = re.search(r"encoder\.layers\.(\d+)\.", name)
        if match:
            stage_idx = int(match.group(1))
            if stage_idx < len(per_stage_bits):
                w_bit = int(per_stage_bits[stage_idx])
            else:
                w_bit = int(per_stage_bits[-1])  # fallback
        else:
            # fallback for patch embedding or other modules not in a stage
            w_bit = int(per_stage_bits[0])

        total_bits += param.numel() * w_bit
...
def compute_ucb_bounds(risks, delta, B, n):
    import math
    # For simplicity, assume a simple bound: R_plus = Rhat + B * sqrt((2 log(1/delta))/n)
    correction = B * math.sqrt((2 * math.log(1/delta))/n)
    R_plus = {idx: risks[idx] + correction for idx in range(len(risks))}
    return R_plus

def pick_threshold_empirical(exit_risks_avg, epsilon):
    """
    Empirical approach: choose threshold that achieves desired risk ≤ epsilon.
    Since we map threshold as (avg_risk + epsilon), 
    you might just pick avg_risk directly or sort exits by avg_risk and pick smallest one ≤ epsilon.
    """
    # Sort exits by their risk
    sorted_exits = sorted(enumerate(exit_risks_avg), key=lambda x: x[1])
    # Find first exit whose risk ≤ epsilon
    for idx, risk in sorted_exits:
        if risk <= epsilon:
            # threshold = risk + epsilon as per original logic
            return risk + epsilon
    # If none found, fallback to largest
    return 1.0

def pick_threshold_crc(exit_risks_avg, epsilon, B, n):
    """
    CRC approach: choose smallest avg_risk s.t. (n/(n+1))*Rhat(λ) + B/(n+1) ≤ ε
    Solve for Rhat(λ): Rhat(λ) ≤ (ε(n+1)-B)/n
    """
    target = (epsilon*(n+1)-B)/n
    # Sort exits by risk
    sorted_exits = sorted(exit_risks_avg)
    for risk in sorted_exits:
        if risk <= target:
            return risk + epsilon
    return 1.0

def pick_threshold_ucb(exit_risks_avg, epsilon, B, n, delta):
    """
    UCB approach: compute upper confidence bounds and pick smallest λ 
    s.t. for all λ'≥λ, R_plus(λ') < ε
    """
    R_plus = compute_ucb_bounds(exit_risks_avg, delta, B, n)
    # Sort by index or risk
    sorted_exits = sorted(enumerate(exit_risks_avg), key=lambda x: x[1])
    for i, (idx, risk) in enumerate(sorted_exits):
        # Check condition that for all λ' ≥ λ, R_plus(λ') < ε
        # Since sorted by risk, just check from this exit onwards
        if all(R_plus[jdx] < epsilon for jdx, _ in sorted_exits[i:]):
            return risk + epsilon
    return 1.0
...
def calibrate_thresholds(
    ...
    """
    Calibrate a threshold for each early-exit head using a static, hard-coded bitwidth->SQNR mapping.
    Steps:
      1) Collect average 'risk' at candidate lambdas for each exit.
      2) For each exit, retrieve bitwidth from a fixed map (bitwidth_map[exit_x]).
      3) Retrieve the corresponding SQNR from another map (sqnr_map[exit_x]).
      4) sensitivity = 1/SQNR (or any function of SQNR).
      5) Pick the smallest lambda such that: risk(lambda) + alpha * sensitivity <= epsilon.
    """
    ...
    # Discrete candidate thresholds
    candidate_lambdas = np.linspace(0, 1, 21)  # e.g. [0.0, 0.05, 0.1, ..., 1.0]

    # Hard-coded bitwidth and SQNR maps for each exit. You can adapt these names/keys as needed:
    bitwidth_map = {
        "exit_3": 8,
        "exit_6": 8,
        "exit_9": 7,
    ...
    thresholds = []       # Final per-exit thresholds
    exit_names = []       # Keep track of which "exit_x" each threshold corresponds to
    exit_counter = 0      # Global index for each exit across all stages

    # Simple function for computing performance gap risk
    def performance_gap_risk(hwy_logits, final_logits, labs):
        loss_exit = F.cross_entropy(hwy_logits, labs)
        #print(f"loss_exit={loss_exit}")
        loss_full = F.cross_entropy(final_logits, labs)
        #print(f"loss_full={loss_full}")
        return loss_exit - loss_full

    # Go through each stage in the model
    for stage_idx, layer in enumerate(model.swin.encoder.layers):
        num_exits = len(getattr(layer, 'highway', []))
        if num_exits == 0:
            continue

        print(f"\n[Calibrate] Stage {stage_idx+1}, #Exits = {num_exits}")

        # We track total risk across discrete lambdas for each exit
        exit_risks_accum = [[0.0 for _ in candidate_lambdas] for _ in range(num_exits)]
        exit_counts = [0 for _ in range(num_exits)]

        # ----------- 1) Forward pass on calibration data, accumulate risk ----------- #
        for _, batch in enumerate(dataloader):
            inputs, labels = batch["pixel_values"], batch["labels"]
            inputs, labels = inputs.to(device), labels.to(device)

            with torch.no_grad():
                # final-layer logits from the full model (disable early exits)
                final_outputs = full_model(pixel_values=inputs, disable_early_exits=True)
            final_layer_logits = final_outputs.logits
            if final_layer_logits is None:
                continue

            # Forward partial model up to this stage
            with torch.no_grad():
                embedding_output, input_dimensions = model.swin.embeddings(inputs)
                hidden_states = embedding_output

                # Pass earlier stages fully
                for prev_stage_idx in range(stage_idx):
                    prev_stage = model.swin.encoder.layers[prev_stage_idx]
                    stage_outs = prev_stage(
                        hidden_states,
                        input_dimensions,
                        disable_early_exits=True,
                        labels=labels,
                        final_layer_logits=None
                    )
                    hidden_states = stage_outs[0]
                    output_dimensions = stage_outs[2]
                    input_dimensions = (output_dimensions[-2], output_dimensions[-1])

                # Gather block outputs in this stage
                block_hidden_states_cache = {}
                for blk_idx, block in enumerate(layer.blocks):
                    blk_outs = block(hidden_states, input_dimensions, always_partition=False)
                    hidden_states = blk_outs[0]
                    block_hidden_states_cache[blk_idx] = hidden_states

            # For each exit in this stage, compute risk
            if hasattr(layer, 'position_exits'):
                position_exits = layer.position_exits
            else:
                position_exits = {}

            for exit_idx in range(num_exits):
                exit_block_idx = None
                for blk_idx, hwy_idx in position_exits.items():
                    if hwy_idx == exit_idx:
                        exit_block_idx = blk_idx
                        break
                if exit_block_idx not in block_hidden_states_cache:
                    continue

                exit_hidden_states = block_hidden_states_cache[exit_block_idx]
                with torch.no_grad():
                    highway_output = layer.highway[exit_idx]((exit_hidden_states,))
                if not isinstance(highway_output, tuple) or len(highway_output) < 1:
                    continue

                exit_logits = highway_output[0]
                if exit_logits is None:
                    continue

                # Compute gap risk
                risk_tensor = performance_gap_risk(exit_logits, final_layer_logits, labels)
                risk_val = risk_tensor.item()
                batch_sz = labels.size(0)

                # Compute confidence
                with torch.no_grad():
                    confidence_exit = exit_logits.softmax(dim=-1).max(dim=-1)[0]

                # Accumulate risk for each candidate lambda
                for lam_i, lam_val in enumerate(candidate_lambdas):
                    mask = (confidence_exit >= lam_val)
                    num_exiting = mask.sum().item()
                    exit_risks_accum[exit_idx][lam_i] += risk_val * num_exiting

                exit_counts[exit_idx] += batch_sz

        # ----------- 2) Average risk for each exit, across candidate lambdas ----------- #
        final_exit_lambda_risks = []
        for e_i in range(num_exits):
            lam_risks = []
            if exit_counts[e_i] > 0:
                for lam_i in range(len(candidate_lambdas)):
                    lam_risks.append(exit_risks_accum[e_i][lam_i] / exit_counts[e_i])
            else:
                lam_risks = [0.0] * len(candidate_lambdas)
            final_exit_lambda_risks.append(lam_risks)

        # ----------- 3) Pick threshold using static bitwidth & SQNR maps ----------- #
        for e_i in range(num_exits):
            # Construct a name for the exit, e.g., "exit_{block}" or "exit_{(stage, e_i)}"
            # For demonstration, we do: exit_{global_exit_counter}, or you can parse from the model
            exit_name = f"exit_{exit_counter+1}"
            # If you know the actual label from your table, map it here:
            # e.g., you might do a dictionary of which exit_name corresponds to which line in your table.

            # For example, if you do a simple approach: stage # + e_i => "exit_3", "exit_6", ...
            # We'll just do a fallback approach to show how to connect with your bitwidth_map:
            # If you have them exactly matched by index, you can do something like a small dictionary or logic here:
            # e.g. if exit_counter == 0, exit_name = "exit_3", etc.
            # Hard-coding example:
            if exit_counter == 0:
                exit_name = "exit_3"
            elif exit_counter == 1:
                ...
                exit_name = f"exit_{exit_counter+1}"  # fallback

            # Retrieve bitwidth from bitwidth_map (hard-coded)
            if exit_name in bitwidth_map:
                bit_i = bitwidth_map[exit_name]
            else:
                bit_i = 8  # fallback if not found

            # Retrieve SQNR from sqnr_map
            if exit_name in sqnr_map:
                sqnr_val = sqnr_map[exit_name]
            else:
                sqnr_val = 1.0

            # sensitivity = 1.0 / sqnr
            sensitivity = 1.0 / max(sqnr_val, 1e-6)

            # pick threshold
            chosen_threshold = 1.0
            lam_risks = final_exit_lambda_risks[e_i]
            
            for lam_i, risk_val in enumerate(lam_risks):
                print(f"lamda_i={lam_i},risk_val={risk_val},sensitivity={sensitivity}")
                if (alpha *  risk_val + beta * sensitivity) <= epsilon:
                    chosen_threshold = candidate_lambdas[lam_i]
                    final_risk = risk_val
                    break

            thresholds.append(chosen_threshold)
            exit_names.append(exit_name)

            print(
                f"Stage={stage_idx+1}, ExitIndex={e_i}, GlobalExitIdx={exit_counter}, "
                f"ExitName={exit_name}, bit={bit_i}, final_risk={final_risk:.2f}, SQNR={sqnr_val:.2f}, "
                f"S={sensitivity:.4f}, threshold={chosen_threshold:.2f}"
            )
            exit_counter += 1
    ...
    for i, thr in enumerate(thresholds):
        print(f"({i}) {exit_names[i]} -> threshold={thr:.2f}")
    ...
    def evaluation_loop(
        ...
        for i in range(1, 25):
            ...
            if key in metrics:
                ...
                exit_counts[i] = 0
        fake_size_mb = approximate_quantized_model_size_staged(model)
        ...
        metrics[f"{metric_key_prefix}_model_size"] = fake_size_mb
        # Prefix all keys
...
def main():
    ...
    dythreshold = model_args.dythreshold
    ...
    if dythreshold:
        output_dir_name = f"Swin_quant_{quant_bits}_risk_acc_w_bits_{per_w_layer_bits_str}_a_bits_{per_a_layer_bits_str}__model_size"
    else:
        output_dir_name = f"Swin_quant_{quant_bits}_conf_{confidence}_w_bits_{per_w_layer_bits_str}_a_bits_{per_a_layer_bits_str}_model_size"
    #output_dir_name = f"Swin_quant_{quant_bits}_risk_acc_w_bits_{per_w_layer_bits_str}_a_bits_{per_a_layer_bits_str}"
    ...
    if model_args.quantize_model:
       ...
       # thresholds = calibrate_thresholds(model, calibration_dataloader,full_model)
        ...
        if dythreshold:
            ...
            #if max_val != min_val:
            #    normalized_thresholds = [(t - min_val) / (max_val - min_val) for t in thresholds]
            #else:
                # In case all risks are equal (very unlikely), default to all zeros or all 0.5
                #normalized_thresholds = [0.5 for _ in thresholds]
            print
            model.set_exit_norm_thresholds(thresholds)

            print("Raw thresholds:", thresholds)
            #print("Normalized thresholds:", normalized_thresholds)
        else:
            # Suppose you have total N early exits in your model
            N = len(thresholds)  # or however you know the total number of exits
            same_thresholds = [confidence] * N
            model.set_exit_thresholds(same_thresholds)
