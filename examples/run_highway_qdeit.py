...
from models.deit_highway.modeling_highway_qdeit import DeiTHighwayForImageClassification
...
def compute_bops(model_config, eval_exit_counts):
    """
...
def approximate_deit_model_size_mb(model, exclude_classifier=False):
    """
    Returns an approximate "fake quantized" size (in MB) assuming each parameter
    is stored at the bitwidth specified by model.config.per_layer_bits for that layer
    instead of float32.

    Args:
        model: Your DeiT model instance (e.g. DeiTForImageClassification or a derived class).
        exclude_classifier: If True, skip final classification heads ("classifier", "cls_classifier", etc.).

    Returns:
        A float representing the approximate size in MB.
    """
    config = model.config
    if not hasattr(config, "per_layer_bits"):
        raise ValueError("model.config.per_layer_bits is not defined.")

    per_layer_bits = config.per_layer_bits  # e.g. [8,8,8,8,6,6,4,4,4,4, ...]
    total_bits = 0

    # Iterate over all named parameters
    for name, param in model.named_parameters():
        # 1) Optionally skip classifier heads
        if exclude_classifier and (
            "classifier" in name or 
            "cls_classifier" in name or
            "distillation_classifier" in name
        ):
            continue

        # 2) Attempt to parse the layer index from something like:
        #      "deit.encoder.layer.7.attention.attention.query.weight"
        match = re.search(r"deit\.encoder\.layer\.(\d+)\.", name)
        if match:
            layer_idx = int(match.group(1))
            # Safety check, in case layer_idx >= len(per_layer_bits)
            if layer_idx < len(per_layer_bits):
                w_bit = per_layer_bits[layer_idx]
            else:
                w_bit = per_layer_bits[-1]
        else:
            # Fallback for patch embeddings, final layernorm, etc.
            # Use bitwidth of layer 0 or some default.
            w_bit = per_layer_bits[0]

        num_params = param.numel()
        total_bits += num_params * w_bit
...
class DeiTConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        quant_bits=8,
        per_layer_bits=None,  # New parameter for per-layer bits
        ...
        **kwargs
        ...
        self.per_layer_bits = per_layer_bits  # Assign per-layer bits
...
class ModelArguments:
    ...
    calibration_steps: int = field(
        default=100,
        metadata={"help": "Number of calibration steps."}
    )
...
"""
def create_calibration_dataset(dataset, num_samples=1024, seed=42):
...
class TrainerwithExits(Trainer):
    ...
    def evaluation_loop(
        ...
        self.sqnr_accumulated_results = [[] for _ in range(num_blocks)]
        ...
        if self.model_args.per_layer_bits:
             logger.info(f"  Quantization setting = {self.model_args.per_layer_bits}")
        ...
        for step, inputs in enumerate(dataloader):
            ...
            #model.device = torch.device('cpu')  # Dynamic quantization is optimized for CPU
                    ...
                    if hidden_states is not None:
                        ...
                        for block_idx, hidden_state in enumerate(hidden_states):
                            if hidden_state is not None:
                                ...
                                self.sqnr_accumulated_results[block_idx].append(sqnr_value)
                            else:
        ...
        if num_samples == 0 and observed_num_examples > 0:
            num_samples = observed_num_examples
        if self.sqnr:
            for block_idx, sqnr_values in enumerate(self.sqnr_accumulated_results):
                if sqnr_values:  # 確保不計算空列表
                    average_sqnr = sum(sqnr_values) / len(sqnr_values)
                else:
                    average_sqnr = float('nan')  # 如果沒有數值，顯示為 nan
                print(f"Average SQNR for Block {block_idx + 1}: {average_sqnr:.2f} dB")
        ...
        fake_size_mb = approximate_deit_model_size_mb(self.model)

        # 2) store it in the metrics dictionary
        metrics[f"{metric_key_prefix}_fake_size_mb"] = fake_size_mb
...
def main():
    ...
    output_dir_name = f"Deit_quant_{quant_bits}_dataset_{data_args.dataset_name}_conf_{confidence}_bits_{per_layer_bits_str}_model_size"

    # Update the output_dir in TrainingArguments
    training_args.output_dir = output_dir_name
    #print("-----------------------------------------training_args.no_cuda -----------------------------------------\n",training_args.no_cuda )
    ...
    if len(per_layer_bits) != config.num_hidden_layers:
        raise ValueError(f"The number of per-layer bits ({len(per_layer_bits)}) does not match the number of hidden layers ({config.num_hidden_layers}).")

    print(f"Per-layer bits: {per_layer_bits}")
    ...
    #print_model_size(quantized_model, "quantized_model")
