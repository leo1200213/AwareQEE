...
class SwinEmbeddings(nn.Module):
    ...
    def __init__(self, config, use_mask_token=False):
        ...
        self.patch_embeddings = qSwinPatchEmbeddings(config)
...
class qSwinPatchEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: Optional[torch.FloatTensor]) -> Tuple[torch.Tensor, Tuple[int]]:
        ...
        # Only perform the check if num_channels is an integer (i.e., not a symbolic tensor)
        if isinstance(num_channels, int):
            if num_channels != self.num_channels:
                raise ValueError(
                    f"Channel dimension of the pixel values ({num_channels}) does not match the number of input channels ({self.num_channels})."
                )
        else:
...
class SwinPatchMerging(nn.Module):
    ...
    def maybe_pad(self, input_feature, height, width):
        # Only perform padding if height and width are integers
        if isinstance(height, int) and isinstance(width, int):
...
class SwinSelfAttention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size, layer_idx: int):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError(
        ...
        w_bit_width = int(config.per_w_layer_bits[layer_idx])
        a_bit_width = int(config.per_a_layer_bits[layer_idx])
        ...
        self.matmul_qk = MinMaxQuantMatMul(A_bit=a_bit_width, B_bit=w_bit_width, mode='raw')
        self.matmul_av = MinMaxQuantMatMul(A_bit=a_bit_width, B_bit=w_bit_width, mode='raw')
    ...
    def forward(
        ...
        #bit_width = int(config.per_layer_bits[layer_idx])
        ...
        context_layer = self.matmul_av(attention_probs, value_layer)
...
class SwinSelfOutput(nn.Module):
    def __init__(self, config, dim, layer_idx: int):
        super().__init__()
        w_bit_width = int(config.per_w_layer_bits[layer_idx])
        a_bit_width = int(config.per_a_layer_bits[layer_idx])
        #self.dense = nn.Linear(dim, dim)
        self.dense = MinMaxQuantLinear(
            dim,
            dim,
            bias=config.qkv_bias,
            mode='raw',
...
class SwinAttention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size, layer_idx: int):
        super().__init__()
        self.self = SwinSelfAttention(config, dim, num_heads, window_size, layer_idx)
        self.output = SwinSelfOutput(config, dim, layer_idx)
...
class SwinLayer(nn.Module):
    def __init__(self, config, dim, input_resolution, num_heads, layer_idx ,shift_size=0):
        super().__init__()
        ...
        self.attention = SwinAttention(config, dim, num_heads, window_size=self.window_size,layer_idx = layer_idx)
        self.drop_path = SwinDropPath(config.drop_path_rate) if config.drop_path_rate > 0.0 else nn.Identity()
        self.layernorm_after = nn.LayerNorm(dim, eps=config.layer_norm_eps)
        self.intermediate = SwinIntermediate(config, dim,layer_idx)
        self.output = SwinOutput(config, dim, layer_idx)
    """
    ...
    """
    def set_shift_and_window_size(self, input_resolution):
        # Only perform the check if input_resolution contains integers
        if isinstance(input_resolution[0], int) and isinstance(input_resolution[1], int):
            if min(input_resolution) <= self.window_size:
                # If window size is larger than input resolution, we don't partition windows
                self.shift_size = 0
                self.window_size = min(input_resolution)
...
class SwinStage(nn.Module):
    def __init__(self, config, dim, input_resolution, depth, num_heads, drop_path, downsample, layer_idx: int):
        super().__init__()
        self.config = config
        self.dim = dim
...
class SwinEncoder(nn.Module):
    def __init__(self, config, grid_size, layer_idx: int):
        super().__init__()
