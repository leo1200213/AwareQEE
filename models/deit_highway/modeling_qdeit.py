...
class DeiTSelfAttention(nn.Module):
    def __init__(self, config: DeiTConfig, layer_idx: int) -> None:
        super().__init__()
        ...
        bit_width = config.per_layer_bits[layer_idx]
        print(f"------------------------------------layer_idx={self.layer_idx},bit_width={bit_width}\n")
        #print(f"Initializing DeiTSelfAttention Layer {layer_idx} with bit-width: {bit_width}")
        ...
        self.matmul_qk = MinMaxQuantMatMul(A_bit=bit_width, B_bit=bit_width, mode='raw')
        self.matmul_av = MinMaxQuantMatMul(A_bit=bit_width, B_bit=bit_width, mode='raw')
...
class DeiTAttention(nn.Module):
    def __init__(self, config: DeiTConfig, layer_idx: int) -> None:
        super().__init__()
        self.attention = DeiTSelfAttention(config, layer_idx)
        self.output = DeiTSelfOutput(config, layer_idx)
...
class DeiTIntermediate(nn.Module):
    def __init__(self, config: DeiTConfig,layer_idx) -> None:
        super().__init__()
...
class DeiTLayer(nn.Module):
    ...
    def __init__(self, config: DeiTConfig, layer_idx: int) -> None:
        ...
        self.attention = DeiTAttention(config,layer_idx)
        self.intermediate = DeiTIntermediate(config,layer_idx)
        self.output = DeiTOutput(config,layer_idx)
