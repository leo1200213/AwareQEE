...
from modeling_qdeit import DeiTEmbeddings, DeiTLayer, DeiTPreTrainedModel, DeiTPatchEmbeddings
from configuration_qdeit import DeiTConfig
from models.deit_highway.qhighway import ViTHighway, DeiTHighway, DeiTHighway_v2, ViT_EE_Highway
...
class DeiTEncoder(nn.Module):
    def __init__(self, config: DeiTConfig):
        ...
        self.layer = nn.ModuleList(
            [DeiTLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)]
        )
    ...
    def set_early_exit_position(self):
        ...
        print('num_early_exits is =====', self.num_early_exits)
                ...
                if not self.training:
                    ...
                    if self.exit_strategy == 'entropy':
                        ...
                        if highway_entropy < self.early_exit_threshold[self.position_exits[i]]:
                            print(f"Early exit at layer {i+1}")
                            ...
                            # Include all_hidden_states in the output
                            if output_hidden_states and hidden_states is not None:
                                all_hidden_states = all_hidden_states + (hidden_states,)

                            # Construct the new_output
                            new_output = (highway_logits,)
                            if output_hidden_states:
                                new_output += (all_hidden_states,)
                            if output_attentions:
                                new_output += (all_self_attentions,)
                            new_output += ({"highway": all_highway_exits},)
    ...
    def forward(
       ...
       # print('----------------------------------------check hidden state----------------------------------------\n')
        ...
        try:
            ...
            # Adjust outputs to include hidden states correctly
            if output_hidden_states and len(outputs) > 2:
                all_hidden_states = outputs[1]
                outputs = (logits, all_hidden_states) + outputs[2:]
            else:
                all_hidden_states = None
                outputs = (logits,) + outputs[1:]

            # Include all_hidden_states in outputs if they exist
            if all_hidden_states is not None:
                outputs = outputs[:1] + (all_hidden_states,) + outputs[1:]
    ...
    def forward(
        ...
        if self.training:
            ...
            try:
                ...
                if output_hidden_states:
                    all_hidden_states = outputs[1]
                    outputs = (logits,) + outputs[2:]  # Skip the hidden_states in outputs[1]
                else:
                    all_hidden_states = None
                    outputs = (logits,) + outputs[1:]

                # Now, outputs should have the same structure as in the normal case
                # If you need to include `all_hidden_states` in outputs for further processing
                if all_hidden_states is not None:
                    outputs = outputs[:1] + (all_hidden_states,) + outputs[1:]
            ...
            if labels is not None:
                ...
                # If labels are None, we skip computing the loss
                outputs = outputs
            outputs = outputs[:-1] + ((original_score, highway_score), exit_layer)
...
"""def cal_hete_loss(stu_fea, tea_fea):
    #print(f"stu_fea shape: {stu_fea.shape}")  # e.g., (64, 768)
    #print(f"tea_fea shape: {tea_fea.shape}")  # e.g., (64, 196, 768)

    # Handle cases based on actual shapes
    if stu_fea.dim() == 2 and tea_fea.dim() == 3:
        # Adjust stu_fea to match tea_fea's dimensions
        stu_fea = stu_fea.unsqueeze(1)  # Now (64, 1, 768)
        stu_fea = stu_fea.expand(-1, tea_fea.size(1), -1)  # Now (64, 196, 768)

    # Proceed with permutation if necessary
    stu_fea = stu_fea.permute(0, 2, 1)  # Now (64, 768, 196)
    tea_fea = tea_fea.permute(0, 2, 1)  # Ensure tea_fea is permuted similarly
...
def cal_hete_loss(stu_fea, tea_fea):
    # Aggregate tea_fea
    tea_fea = tea_fea.mean(dim=1)
    
    # Convert to probabilities
    stu_fea = F.log_softmax(stu_fea, dim=-1)
    tea_fea = F.softmax(tea_fea, dim=-1)
    
    # Compute KL divergence
    loss = F.kl_div(stu_fea, tea_fea, reduction="batchmean")
    return loss